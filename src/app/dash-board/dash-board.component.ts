...
export class DashBoardComponent {
  ...
  ngOnInit(): void {
    ...
    this.coinService.getNews().subscribe((data) => { this.news = data; });
  }
    ...
    if (this.userSignedIn) {
      if (this.currentUser) {
        ...
        this.userService.addCoinToWatchlist(watchItem).subscribe((result) => {
          let clickedCoin = _.find(this.referCoins, (item) => item.coinId == coinId);
  ...
  buyCoin(coin:any) {
    if (this.currentUser) {
      let coinItem = { CoinId: coin.coinId, UserId: this.currentUser.userId };
      this.userService.buyCoin(coinItem).subscribe((result) => {
        this.sharedService.ValidateUserProfile();
        alert("The coin was added in your shopping cart.");
      });
    } else
      alert("Please sign in first.");
  }
  ...
  openCoinDetail(coinId: number): void {
    const dialogRef = this.dialog.open(CoinDetailDialogComponent, { width: '80%', height: '80%', data: { View: _.find(this.referCoins, (item) => item.coinId == coinId) }});
  }
