...
export class CoinListPageComponent {
  ...
  getCoinsByPage(needConcat: boolean) {
    this.coinService.getCoinsByPage({ UserId: this.currentUser?.userId, Skip: this.skip, PageSize: this.pageSize, CoinId: 0, ParaString: this.searchWord, Filter: this.currentFilter.value, Sort: this.currentSortBy.value }).subscribe((data) => {
      $("#divSpin").hide();
      if (needConcat)
  ...
  sortChange(value: any): void {
    ...
    this.currentSortBy.value = value;
    setTimeout(() => { this.getCoinsByPage(false); }, 50);
  }
  ...
  onScroll() {
    ...
    console.log(div.scrollTop + div.offsetHeight + "->" + (div.scrollHeight - 100));
    if (div.scrollTop > 0) {
  ...
  public addNewCoin() { window.location.href = window.location.href.replace("CoinList", "NewCoin"); }
